...
async fn handle_sync(
    ...
    let desktop_sync_data = match export_result {
        ...
        Err(e) => {
            let _ = state
                .app_handle
                .emit("sync_error", format!("Erro ao exportar: {}", e));
            return Json(SyncResponse {
                success: false,
    ...
    if !payload.sync_data.is_empty() && payload.sync_data != "{}" {
        ...
        match import_result {
            Ok(msg) => {
                let _ = state
                    .app_handle
                    .emit("sync_status", format!("Sincronização concluída: {}", msg));
            }
            Err(_e) => {
                let _ = state.app_handle.emit(
                    "sync_status",
                    format!(
                        "Aviso: Recebeu conexão, mas ignorou importação (dados inválidos/vazios)"
                    ),
                );
            }
        }
        ...
        let _ = state.app_handle.emit(
            "sync_status",
            "Aviso: Conexão recebida, mas sem dados para importar.".to_string(),
        );
    }
